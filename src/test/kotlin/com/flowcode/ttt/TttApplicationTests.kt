...
@DataJpaTest
class TttApplicationTests (@Autowired val entityManager: TestEntityManager,
	...
	fun `Return Game by GameId`() {
		...
		assertThat(found.get()).isEqualTo(game)
	}
	...
	fun `Return User by UserId`() {
		...
		assertThat(found.get()).isEqualTo(florian)
	}
	...
	fun `Return Move by MoveId`() {
		...
		val game = Game(florian,ada,"pending", "ranked", 'X')
		entityManager.persist(game)
		val move = Move(florian, game, '1', 2, 1)
		entityManager.persist(move)
		entityManager.flush()

		val found = moveRepository.findById(move.id!!)

		assertThat(found.get()).isEqualTo(move)
	}
	...
	fun `Returns Moves by Player and Game`() {
		...
		entityManager.persist(game)
		val move1 = Move(florian, game, '1', 2, 1)
		val move2 = Move(ada, game, '5', 3, 3)
		val move3 = Move(florian, game, '9', 1, 1)
		val move4 = Move(ada, game2, '5', 3, 3)
		val move5 = Move(florian, game2, '9', 1, 1)
	...
	fun `Returns Moves by Game`() {
		...
		assertThat(foundSecond.size).isEqualTo(2)
	}
